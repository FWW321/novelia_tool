...
const QueueGPTV2: ModuleDefinition = {
  name: '排队GPT v2',
  type: 'onclick',
  ...
  run: async (cfg) => {
    ...
    switch (type) {
      case 'wenkus': {
        ...
        await StorageService.addJobs(StorageService.gptKey, results);
        break;
      }
      case 'wenku': {
      ...
      case 'novel': {
        try {
          ...
          errorFlag = true;
          NotificationService.showError(`Failed to fetch data for ${document.title}.`);
        }
        break;
      }
      case 'favorite-web': {
        ...
        while (true) {
          ...
          try {
            ...
            await StorageService.addJobs(StorageService.gptKey, tasks);
            results.push(...tasks);
            NotificationService.showSuccess(
              `成功排队 ${3 * page + 1}-${3 * page + 3}页, 共${novelCount}本小说`
            );
          } catch (error) {
            console.log(error);
